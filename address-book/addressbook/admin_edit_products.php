...
if(isset($_GET['id'])){
    ...
    $r = mysqli_query($connect,"select * from products where P_id=$pid");
    if(mysqli_num_rows($r) > 0){

        while($row = mysqli_fetch_array($r)){
            $img = $row['P_imgpath'];
            $name = $row['P_productName'];
            $price = $row['P_price'];
            $mcat = $row['P_mainCategory'];
            $scat = $row['P_subCategory'];

        }
    }
    else{
        header('location: admin_products.php?err=Product Not Found');
    exit();    
    }
}
else{
    header('location: admin_products.php?err=Product Id Not Found');
    exit();
}
?>
<!DOCTYPE html>
            ...
            <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    ...
                    <p class="card-description">For Edit </p>
                    <form class="forms-sample" action="layout/products/edit.php" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="P_id" value="<?php echo $pid; ?>">
                      <div class="form-group">
                        <label for="exampleInputName1">Product Name</label>
                        <input type="text" class="form-control" id="exampleInputName1" placeholder="Name" name="P_name" value="<?php echo $name; ?>">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputName1">Product Price</label>
                        <input type="text" class="form-control" id="exampleInputName1" placeholder="Price" name="P_price" value="<?php echo $price; ?>">
                      </div>
                      <div class="form-group">
                        <label for="exampleSelectGender">Main Category</label>
                        <select class="form-control" id="exampleSelectGender" name="m_category">
                        <option name="m_category" value="<?php echo $mcat; ?>"><?php echo $mcat; ?></option>
                        <?php
                        require 'layout/dbcon.php';
                    ...
                    ?>
                      ...
                      <div class="form-group">
                        ...
                        <option name="m_category" value="<?php echo $scat; ?>"><?php echo $scat; ?></option>
                        <?php
                    ...
                    ?>
                      ...
                      <button type="submit" class="btn btn-gradient-primary mr-2" name="product_update_btn">Submit</button>
